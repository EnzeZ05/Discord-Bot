...
command: '`artifact <artifactName(berserker) no spaces(luckydog)> <artifactType(flower, sands, circlet, plume, goblet)>`',    
async execute( message, args, client, Discord) {
        if (args.length > 0) {
            
            const joined = args[0];
            console.log({joined, args1: args[1], artJoined: genshin.artifacts(joined)});
            const answerEmbed = new Discord.MessageEmbed()
                .setColor('#FFF18E')
                .setTitle("Artifact-" + genshin.artifacts(joined)[args[1]].name)
                .setDescription(genshin.artifacts(joined)[args[1]].description)
                .setThumbnail(genshin.artifacts(joined).images[args[1]])
                .addFields({
                ...
                .setTimestamp()
                .setFooter('Send \'!yoihelp\' for help with bot commands!');
